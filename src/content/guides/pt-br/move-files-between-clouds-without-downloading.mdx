---
locale: pt-br
title: "Como mover arquivos entre nuvens sem baixá-los antes"
description: "Um guia prático para mover arquivos entre provedores de nuvem — Drive, OneDrive, Dropbox, S3 — sem arrastar cada byte pelo seu notebook."
eyebrow: "Guia"
headline: "Mova arquivos entre nuvens sem baixá-los antes"
...
- **Ferramentas hospedadas de servidor a servidor.** O Mover.io era o exemplo canônico antes de a Microsoft comprar e fechar o serviço para destinos não-Microsoft. MultCloud, CloudHQ e serviços parecidos ainda existem; variam bastante em preço, cobertura de provedores e em quão transparentes são sobre por onde seus dados passam.

Para uma transferência pontual de tamanho moderado, alguma delas provavelmente resolve.

## Quando isso ainda não basta

A resposta honesta para "qual é a melhor" é "depende do job". Algumas situações passam do que as respostas simples cobrem:
...
Se alguma dessas descreve a sua situação, uma CLI mais um cron job ainda funcionam — mas agora você está mantendo infraestrutura de transferência como projeto paralelo. Esse é o vão que ferramentas dedicadas tentam preencher.

## Como a Syncologic faz isso

A Syncologic vai mover arquivos entre nuvens sem nunca puxar tudo pelo seu notebook. O fluxo é o mesmo independentemente do par de provedores:
...
Há três opções aí. O **Cloud Runner** é o padrão prático — a infraestrutura da Syncologic faz o trabalho, com credenciais de curta duração e estado de job isolado. O **Browser Runner** ("This Device") faz os bytes passarem pela aba aberta do seu navegador, então nada vive em um runner hospedado; o trade-off é que a aba vai precisar ficar aberta. O **Private Runner** é um binário pequeno que você roda na sua própria máquina, NAS ou VPS — os bytes fluem origem → seu hardware → destino, e a Syncologic só vê metadados.
