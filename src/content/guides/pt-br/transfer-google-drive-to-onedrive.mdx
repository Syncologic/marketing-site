...
Aí você escolhe onde a transferência vai rodar. O **Cloud Runner** é a opção mais prática — a infraestrutura da Syncologic cuida dos bytes, com credenciais de curta duração e cada job isolado dos outros. O **Browser Runner** ("This Device") passa os bytes pela aba do seu navegador; nada vive num servidor nosso, mas a aba precisa continuar aberta até o fim. O **Private Runner** é um binário pequeno que roda no seu próprio servidor, NAS ou VPS — os bytes vão Drive → seu hardware → OneDrive, e a Syncologic só vê os metadados. Para uma migração de Workspace para Microsoft 365 em que os dados não podem sair do seu controle, costuma ser a escolha certa.
